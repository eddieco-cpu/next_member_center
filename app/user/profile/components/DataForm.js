...
function VerifyEmblem(props) {
  return (
    <div className={`${classes["row--verify-state"]}`}>
      {/* <img src={props.condition ? isBind : noBind} /> */}
      <b
        style={{
...
export default function DataForm() {
  ...
  const [postcodeArr] = useState([
    { id: 1000, des: '請選擇郵遞區號', zip: 1000 }, ...getZipArr()])
  ...
  return (
    <section className={classes.profile__wrapper}>
          ...
          <div className={`${classes.row} ${classes.row_phone}`}>
            ...
            <VerifyEmblem
              condition={
                userData?.mobileconfirm && userData?.mobileconfirm === 'Y'
              }
            />
          </div>
          ...
          <div className={`${classes.row} ${classes["row--birth"]}`}>
            <p className={`${classes.birth} ${classes["birth--des"]}`}>生日</p>
            <TextInput className={`${classes.birth} ${classes["birth--year"]}`}>
              <input
                name="year"
                id="year"
                type="number"
            ...
            <TextInput className={`${classes.birth} ${classes["birth--month"]}`}>
              <input
                name="month"
                id="month"
                type="number"
            ...
            <TextInput className={`${classes.birth} ${classes["birth--date"]}`}>
              <input
                name="date"
                id="date"
                type="number"
          ...
          <Btn className={`${classes.profile__sender}`}>
            確認送出
          </Btn>
          ...
          <div className={`${classes.row} ${classes["row--linker"]}`}>
            {thirdparty.map((el) => (
              <div className={`${classes.linker_item}`} key={el.id}>
                <b className={`${classes.linker_icon} ${classes[`linker_icon--${el.id}`]}`}>
                  <img src={el.img} />
                </b>
                <button
                  className={`${classes.linker_button} ${
                    userData[el.depandcy] === 'Y' ? '' : `${classes["linker_button--no-use"]}`
                  }`}
                  onClick={() =>
                    unbind3rdP(
                      userData[el.depandcy] === 'Y' ? true : false,
                      el.id
                    )
                  }
                >
