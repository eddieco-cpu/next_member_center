...
export default function ClientContainer({ track_id, tracklist }) {
	...
	const trackItem = tracklist.find(
		(el) => el.track_id.toString() === track_id
	);
	...
	return (
		<>
			<PopupFrame {...{ onTogglePrev, onToggleNext }}>
				<div className={`${classes.author__container} ${classes["author__container--popup"]}`}>
					{/* ===============  avater  ================ */}
					<ul className={classes.track__avater}>
						<Avater {...trackItem} needSpecialist={true} hideSubscribeBtn={true}/>
					</ul>
