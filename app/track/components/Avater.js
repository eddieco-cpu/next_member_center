...
export default function Avater(props) {
	...
	if(isSubscribed) {
		return (
			<li className={`${classes['item-listing__photo-circle']} ${classes['circle-pic-item__wrapper']}`}>
			...
			<Link
				...
				className={`${classes["circle-pic-item__substance"]} ${
					props.needSpecialist && classes["circle-pic-item__substance--lifeless"]
				}`}
			>
				...
				<article className={`${classes["circle-pic-item__para"]}`}>
					<h3 className={`${classes["circle-pic-item__title"]}`}>{props.title}</h3>
					<p className={`${classes["circle-pic-item__description"]}`}>{props.author_title}</p>
				</article>
			</Link>
		</li>
		)
